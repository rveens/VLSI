...
\textbf{ENKEL MINIMALE SAMPLE RATE?}
...
\smallskip 
\textbf{I counted 756+20859 bits being used as registers? I cannot trace 114.}
...
\smallskip
One blockram is used. On our platform (Spartan-6) a 'blockram' contains 18Kb of memory. (\textbf{REFER TO SOURCE? $http://www.xilinx.com/support/documentation/data_sheets/ds160.pdf$})
We only use one blockram, because we store the coefficients that are DWIDTH$*$CWIDTH$=16*L*4=16*160*4=16*640=10240$ bits.

\smallskip
38 IO pins are used, 16*2 for input and output data and 6 extra for ack\_in, ack\_out, req\_in, req\_out, clk and rst.
...
The input and output lengths are 8.454 and 8.455 seconds respectively. The output is 0.00011\% shorter. The reason of this peculiarity is currently unknown.

\textbf{FREQ SPECTRUM BESCHRIJVEN}
...
\end{figure}
\begin{figure}[h]
	\includegraphics[scale = 0.40]{Images/simulation_outputdifference.png}
    \caption{The last part of the input and output, zoomed in. The length of the output is less then the input.}
    \label{fig:outlengdif}
\end{figure}
\begin{figure}[h]
	\begin{subfigure}[b]{0.5\textwidth}	
		\includegraphics[scale=0.40]{Images/simulation_frequencyspectrum_input.png}
	\end{subfigure}
	\begin{subfigure}[b]{0.5\textwidth}	
		\includegraphics[scale=0.40]{Images/simulation_frequencyspectrum_output.png}
	\end{subfigure}
